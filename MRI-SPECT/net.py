...
class Multiscale_VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        assert x.size()[1]==1 and x.size()[2]==256 and x.size()[3]==256,f"image x.size must be 256*256 and 1 channel"
        ...
        x, h3, w3 = self.patch_embed3(x)  # [B,h3*w3,128]
        ...
        x, h4, w4 = self.patch_embed4(x)  # [B,h4*w4,128]
...
class net2(nn.Module):
    def __init__(self):
        ...
        self.prelu=nn.PReLU()
        self.dilate_conv1=ConvLayer1(32,32,1,1,2)
        self.dilate_conv2=ConvLayer1(32,64,3,1,2)
        self.dilate_conv3=ConvLayer1(32,96,5,1,2)
        self.res2block1 = res2net(Bottle2neck,32,32,1)
        self.res2block2 = res2net(Bottle2neck, 64,64,1)
        self.res2block3 = res2net(Bottle2neck, 96, 96,1)
    def forward(self,x):
        x_conv=self.conv(x)
...
class net3(nn.Module):
    def __init__(self):
        ...
        self.conv=ConvLayer(256,192,3,1)
    def forward(self,x):
        l1, l2, l3, l4, l5 = self.vit(x)
        #Global  Semantic Feature Aggregation Module
...
class transform_encoder(nn.Module):
    def __init__(self):
        ...
        self.net2=net2()
        self.net3=net3()
        self.conv=ConvLayer(192,192,3,1)
        self.conv1=ConvLayer(192,192,3,1,is_last=True)
...
class transform_decoder(nn.Module):
    def __init__(self):
        ...
        self.conv1=ConvLayer(192,64,3,1,2)
        self.conv2=ConvLayer(64,64,3,1,2)
        self.conv3=ConvLayer(128,64,3,1,2)
        self.conv4=ConvLayer(128,64,3,1,2)
...
class Net(nn.Module):
    ...
    def Transform_Encoder(self,img):
        res=self.encoder(img)
        return res
    def Transform_Decoder(self,img):
        result=self.decoder(img)
        return result
